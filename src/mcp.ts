...
async function authorizedHttp(request: Request, authorizer: ReturnType<typeof createMcpAuthorizer>, credentialsFor: McpHttpOptions["credentialsFor"], apiBaseUrl: string | undefined): Promise<Response> {
  ...
  if (request.method !== "POST") {
    ...
    // revisions; this stateless transport answers 405 instead.
  ...
  // Validate the selected wire revision before dispatching a request.
  ...
  try {
    outcome = await handleRpc(serverFor(remote), parsed, request.headers.get("mcp-protocol-version"));
  } catch (e) {
...
async function startStdio(): Promise<void> {
  ...
  const dispatch = createStdioRpcHandler(server);
  rl.on("line", (line) => {
